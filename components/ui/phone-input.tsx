...
export const PhoneInput = forwardRef<HTMLInputElement, PhoneInputProps>(
  (
    ...
    const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Prevent erasing the country code if present
      if (countryData && countryData.countryCallingCodes?.[0]) {
        const code = countryData.countryCallingCodes[0];
        if (!newValue.startsWith(code)) {
          // If user tries to erase or change the code, revert to previous value
          // Only allow editing after the code
