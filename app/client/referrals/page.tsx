...
import { Copy, ExternalLink, Plus, Trash2, Edit, Eye, CalendarIcon } from 'lucide-react'
import { format } from 'date-fns'
import { cn } from '@/lib/utils'
import CustomReferralCodeDialog from '@/components/referrals/CustomReferralCodeDialog'
...
export default function ReferralDashboard() {
  ...
  const createCustomReferralCode = async (customCode: string, maxUses: number | null, pointsPerReferral: number, expiresAt: string | null) => {
    try {
      setCreating(true)
      setError('')
      ...
      if (response.ok) {
        setSuccess('Custom referral code created successfully!')
        fetchData()
      } else {
        const errorData = await response.json()
        setError(errorData.error || 'Failed to create custom referral code')
        throw new Error(errorData.error || 'Failed to create custom referral code')
      }
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">Referral Dashboard</h1>
          <p className="text-muted-foreground text-sm sm:text-base">Create and manage your referral codes</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <Button onClick={createReferralCodeInstant} disabled={creating} className="w-full sm:w-auto">
            <Plus className="w-4 h-4 mr-2" />
            <span className="hidden sm:inline">{creating ? 'Creating...' : 'Create Random Code'}</span>
            <span className="sm:hidden">{creating ? 'Creating...' : 'Random Code'}</span>
          </Button>
          <CustomReferralCodeDialog 
            onCreateCode={createCustomReferralCode} 
            isCreating={creating}
          />
        </div>
      </div>
              ...
              <Card key={code.id}>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-sm">
                    <div>
                      <div className="font-medium text-xs sm:text-sm">Uses</div>
                      <div className="text-sm sm:text-base">{code.current_uses}</div>
                    </div>
                    <div>
                      <div className="font-medium text-xs sm:text-sm">Points</div>
                      <div className="text-sm sm:text-base">{code.points_per_referral} per referral</div>
                    </div>
                    <div>
                    ...
                    <div>
                      <div className="font-medium text-xs sm:text-sm">Expires</div>
                      <div className="text-sm sm:text-base">{formatDate(code.expires_at)}</div>
                    </div>
                  </div>
