...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Fetch class details with instructor information
    ...
    if (classData.instructors) {
      classData.instructor_name = `${classData.instructors.first_name || ''} ${classData.instructors.last_name || ''}`.trim()
    }

    return NextResponse.json(classData)
  } catch (error) {
    console.error('Error in classes GET API:', error)
    return NextResponse.json(
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Only allow admin and trainer access for updating classes
    ...
    // Update class using admin client
    ...
    console.error('Error in classes PUT API:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Only allow admin access for deleting classes
    ...
    // Delete class using admin client
    ...
    console.error('Error in classes DELETE API:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
