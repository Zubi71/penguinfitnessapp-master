...
interface TrackersProps {
  onBack: () => void
  clientId?: string // Optional client ID, defaults to personal use
}

const Trackers: React.FC<TrackersProps> = ({ onBack, clientId = 'personal-use' }) => {
  ...
  const fetchWeightData = async () => {
    try {
      ...
      const dataWithChanges: WeightEntry[] = sortedData.map((entry, index) => {
        let change = 0
        if (index > 0) {
  ...
  useEffect(() => {
    fetchWeightData()
  }, [selectedPeriod, endDate, clientId])
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#2a5d90] text-white rounded-lg hover:bg-red-600"
              >
                Add Weight
              </button>
            </div>
