...
const TrainingCycles: React.FC<TrainingCyclesProps> = ({ clientId, onBack }) => {
  ...
  const fetchTrainingCycles = async () => {
    try {
      ...
      const formattedCycles = (cyclesData || []).map(cycle => ({
        ...cycle,
        status: cycle.status || 'active', // Default to 'active' if status doesn't exist
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="bg-white text-[#2a5d90] hover:bg-gray-100 px-4 py-2 rounded-lg flex items-center space-x-2"
              >
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                {cycles.map((cycle) => (
                  <button
                    key={cycle.id}
                    onClick={() => window.location.href = `/trainer/clients/${clientId}/training-cycles/${cycle.id}`}
