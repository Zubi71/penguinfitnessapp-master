-- FINAL TRIGGER FIX
-- This will properly drop the existing trigger and create the missing table
...
-- 3. Create the missing client_points table
...
-- 7. Create a safer function that won't break user creation
CREATE OR REPLACE FUNCTION create_client_points()
RETURNS TRIGGER AS $$
BEGIN
  -- Only create points for clients, not trainers or admins
...
-- 9. Verify the fix
SELECT 'Trigger fixed and table created! User creation should work now.' as status;
